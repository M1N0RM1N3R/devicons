...
import svgShapesToPath from "./svgshapes2svgpath";
...
export class SVGIcons2SVGFontStream extends Transform {
  ...
  _transform(svgIconStream, _unused, svgIconStreamCallback) {
    ...
    saxStream.on("opentag", (tag) => {
      ...
      try {
        ...
        this.emit(
          "error",
          new Error(
            `Got an error parsing the glyph "${glyph.name}": ${
    ...
    saxStream.on("end", () => {
      if (!isGlyphComplete) {
        this.emit(
          "error",
          new Error(`Incomplete SVG data for glyph "${glyph.name}"`)
        );
        return;
      }
  ...
  _flush(svgFontFlushCallback) {
    // Verify all glyphs are complete before flushing
    if (this.glyphs.some((glyph) => !glyph.paths)) {
      this.emit("error", new Error("Some glyphs were not properly parsed"));
      return;
    }
